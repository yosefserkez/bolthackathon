...
type Sponsor = {
  ...
  logoUrl: string;
  websiteUrl: string;
  tier: 'host' | 'platinum' | 'gold' | 'silver';
};
...
const SponsorsSection: React.FC = () => {
  ...
  const hostCompany = {
    name: "bolt.new",
    logoUrl: "https://pbs.twimg.com/profile_images/1880702021122342912/fe9TlQqJ_400x400.jpg",
  ...
  const hostSponsors = sponsors.filter(sponsor => sponsor.tier === 'host');
  ...
  const SponsorLogo: React.FC<{ sponsor: Sponsor; className?: string }> = ({ sponsor, className = '' }) => (
    <motion.div
      ...
      className={`bg-space-blue/40 border border-electric-purple/30 hover:border-neon-cyan/60 rounded-xl p-6 flex items-center justify-center transition-all duration-300 ${className}`}
    >
      <a 
      ...
      >
        <img 
          ...
          className="w-full h-full object-contain filter brightness-100 hover:brightness-110 transition-filter duration-300"
        />
      </a>
    </motion.div>
  );
  ...
  return (
    ...
    <section id="sponsors" className="relative py-20 md:py-32 bg-space-blue overflow-hidden">
      {/* Background glow elements */}
      <div className="absolute top-1/3 left-1/3 w-96 h-96 rounded-full bg-electric-purple/10 blur-[120px] z-0" />
      <div className="absolute bottom-1/3 right-1/3 w-96 h-96 rounded-full bg-neon-cyan/10 blur-[120px] z-0" />
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center mb-20">
            <h2 className="text-3xl md:text-5xl font-display font-bold text-stellar-white mb-6">
              Our <span className="text-neon-cyan">Sponsors</span>
            </h2>
            <p className="text-lg text-stellar-white/70 max-w-3xl mx-auto">
              The World's Largest Hackathon is made possible by the generous support of our sponsors who are committed to fostering innovation in the developer community.
            </p>
          </motion.div>
              
          {hostSponsors.length > 0 && (
            <motion.div variants={itemVariants} className="mb-16">
              <h3 className="text-xl md:text-2xl text-center font-display font-bold text-stellar-white mb-8">
                Hosted by
              </h3>
              <div className="grid grid-cols-1 gap-8">
                {hostSponsors.map((sponsor, index) => (
                  <SponsorLogo key={index} sponsor={sponsor} className="h-40" />
                ))}
              </div>
            </motion.div>
          )}
            ...
            <motion.div variants={itemVariants} className="mb-16">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {goldSponsors.map((sponsor, index) => (
                  <SponsorLogo key={index} sponsor={sponsor} className="h-32" />
                ))}
              </div>
            </motion.div>
          )}
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {silverSponsors.map((sponsor, index) => (
                  <SponsorLogo key={index} sponsor={sponsor} className="h-24" />
                ))}
              </div>
            </motion.div>
          )}
          ...
          <motion.div 
            variants={itemVariants}
            className="mt-24 text-center p-8 md:p-10 rounded-2xl bg-gradient-to-br from-space-blue/60 to-electric-purple/20 border border-electric-purple/30"
          >
            <h3 className="text-2xl md:text-3xl font-display font-bold text-stellar-white mb-4">
              Become a Sponsor
            </h3>
            <p className="text-lg text-stellar-white/80 mb-8 max-w-3xl mx-auto">
              Interested in sponsoring the World's Largest Hackathon? Get your brand in front of thousands of developers and support the next generation of innovators.
            </p>
            <a 
              href="#contact" 
              className="inline-block py-3 px-8 rounded-lg bg-neon-cyan text-space-blue font-display font-bold hover:bg-opacity-90 transition-all duration-300"
            >
              Contact Us
            </a>
          </motion.div>
